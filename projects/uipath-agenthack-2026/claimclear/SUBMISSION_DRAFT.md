...
**How we built it**
UiPath Maestro Case is the orchestration spine; the agents are nodes inside the case, not a standalone app. Agent reasoning uses Claude; the agents are packaged as UiPath Coded Agents. We built and tested the agent pipeline locally on a synthetic claims dataset for fast iteration, then wired it into Maestro with a human approval task. The whole project was built using coding agents — Claude Code for planning and Codex for implementation — documented in our AGENT_BUILD_LOG.
